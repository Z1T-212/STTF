...
class TransformerEncoderLayer_QKV(nn.Module):
    ...
    """
    def __init__(self, embed_dim, num_heads, attn_dropout, res_dropout, activ_dropout, activation):
    ...
    def _get_key_padding_mask(self, max_length, key_len):
        ...
        # 从0开始循环到key_len-1， unsqueeze增加一个维度， expand维度扩展为（key_len,max_length）
...
class TransformerEncoder(nn.Module):
    def __init__(self, embed_dim, pos_flag,pos_dropout, num_heads, attn_dropout, res_dropout, activ_dropout, activation, num_layers):
        super().__init__()
    ...
    def forward(self, x_q, x_k=None, key_len=None):
        """
        Args:
            x_q (Tensor): input to the layer of shape (seq_len, batch, embed_dim)（5，b，192）
            x_k (None or Tensor): if tensor, input to the layer of shape (seq_len', batch, embed_dim)
            key_len (None or Tensor): if Tensor, input to the layer of shape (batch,) 句子长度
        Returns:
            out shape (seq_len, batch, embed_dim)
        """
        # 位置编码
...
class TransformerEncoder_nopos(nn.Module):
    def __init__(self, embed_dim, num_heads, attn_dropout, res_dropout, activ_dropout, activation, num_layers):
        super().__init__()
